      ...
      {#if offers.closed.length}
        <Table
          ...
          body={offers.closed.map((c) => {
            return { task: c.head, bounty: `${Object.values(c.bounty).reduce((acc, c) => acc + c, 0) / 1000}sats`, closedBy: Object.keys(c.completions)
                .filter((v) => c.completions[v].votes)
                .toString(), closedAt: new Date(c.awarded_at * 1000).toGMTString() };
          })} />
      {/if}
      {#if offers.completions.length}
        <Table
          ...
          body={offers.completions.map((c) => {
            return { task: c.head.substring(0, 25) + '...', bounty: `${c.bounty / 1000}sats`, createdAt: new Date(c.completion.time * 1000).toGMTString() };
          })} />
      {/if}
      {#if offers.contributions.length}
        <Table
          ...
          body={offers.contributions.map((c) => {
            return { task: c.head.substring(0, 25) + '...', bounty: `${c.bounty / 1000}sats`, contribution: `${c.contribution / 1000}sats`, createdAt: new Date(c.created_at * 1000).toGMTString() };
          })} />
      {/if}
      {#if offers.offers.length}
        <Table
          ...
          body={offers.offers.map((c) => {
            return { task: c.head.substring(0, 25) + '...', bounty: `${c.bounty / 1000}sats`, completions: c.ncompletions, createdAt: new Date(c.created_at * 1000).toGMTString() };
          })} />
      {/if}
      <!-- <pre>
